...
-- A. מחיקת העדפות עבור פרופיל שאינו קיים
DELETE FROM profile_preferences
WHERE profile_id NOT IN (SELECT profile_id FROM profiles);

-- B. מחיקת מנויים 'inactive' שנסגרו לפני יותר משנה
DELETE FROM subscriptions
WHERE status = 'inactive'
  AND end_date < (CURRENT_DATE - INTERVAL '1 year');

-- C. מחיקת משתמשים בלי מנוי פעיל ובלי פרופילים
DELETE FROM users
WHERE NOT EXISTS (
    SELECT 1 FROM subscriptions 
    WHERE subscriptions.user_id = users.user_id AND subscriptions.status = 'active'
)
AND NOT EXISTS (
    SELECT 1 FROM profiles 
    WHERE profiles.email IN (
        SELECT email FROM subscriptions WHERE subscriptions.user_id = users.user_id
    )
);

-- 3 UPDATEs:

-- A. עדכון סטטוס מנויים שפג תוקפם ל-'inactive'
UPDATE subscriptions
SET status = 'inactive'
WHERE end_date IS NOT NULL
  AND end_date < CURRENT_DATE;

-- B. הגדרת notifications = FALSE אם NULL
UPDATE profile_preferences
SET notifications = FALSE
WHERE notifications IS NULL;

-- C. המרת כל שמות התכנים לאותיות קטנות
UPDATE content
SET Name = LOWER(Name)
